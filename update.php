<?php
  ...
  if (!empty($_POST)) {
    ...
    $sql = "UPDATE esp8266_table_dht11_leds_update SET humidity = ?, status_read_sensor = ?, time = ?, date = ? WHERE id = ?";
    $q = $pdo->prepare($sql);
    $q->execute(array($humidity,$status_read_sensor,$tm,$dt,$id));
    Database::disconnect();
    //........................................ 
    ...
    while ($found_empty == false) {
      ...
      $sql = 'SELECT * FROM esp8266_table_dht11_leds_record WHERE id="' . $id_key . '"';
      $q = $pdo->prepare($sql);
      $q->execute();
    ...
    $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
		$sql = "INSERT INTO esp8266_table_dht11_leds_record (id,board,humidity,status_read_sensor,LED_01,time,date) values(?, ?, ?, ?, ?, ?, ?)";
		$q = $pdo->prepare($sql);
		$q->execute(array($id_key,$board,$humidity,$status_read_sensor,$led_01,$tm,$dt));
    //::::::::
